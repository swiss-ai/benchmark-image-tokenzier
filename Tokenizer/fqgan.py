...
class FQGAN(Tokenizer):
    ...
    def encode(self, tensor: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, Any]]:
        ...
        with torch.no_grad():
            ...
            (
                ...
                [_, _, indices_vis],
                [_, _, indices_sem_mid],
                [_, _, indices_sem_high],
            ) = self.model.encode(tensor)
    ...
    def decode(
        self, indices: Union[torch.Tensor, Dict[str, torch.Tensor]], additional_info: Dict[str, Any] = None
    ) -> torch.Tensor:
        ...
        with torch.no_grad():
            if isinstance(indices, dict):
                ...
                reconstructed = self.model.decode_code(
                    indices_vis, indices_sem_mid, indices_sem_high, vis_shape, sem_mid_shape, sem_high_shape
                )
            else:
                raise ValueError(
                    "FQGAN requires indices as dictionary with 'indices_vis', 'indices_sem_mid', 'indices_sem_high' keys"
                )
        ...
        if isinstance(metrics["additional_info"], dict) and "latent_shapes" in metrics["additional_info"]:
            ...
            if isinstance(indices, dict):
                ...
                metrics["tokens_per_quantizer"] = {
                    key: idx.numel() if isinstance(idx, torch.Tensor) else len(idx) for key, idx in indices.items()
                }
...
def print_metrics(
    ...
    total_tokens,
    total_tokens_vis,
    total_tokens_sem_mid,
    total_tokens_sem_high,
    compression_ratio,
    original_pixels,
...
def process_single_image(tokenizer, tiler, image, name, batch_size, processing_ratio, reconstruction_path):
    ...
    for i in range(0, total_tiles, batch_size):
        ...
        print(
            f"  📦 Processing batch {i//batch_size + 1}/{(total_tiles + batch_size - 1)//batch_size}: tiles {i+1}-{end_idx}"
        )
...
if __name__ == "__main__":
    ...
    for processing_ratio in processing_ratios:
        ...
        if processing_ratio == 1.0:
            ...
            RECONSTRUCTION_PATH = (
                f"/iopsstor/scratch/cscs/xyixuan/benchmark-image-tokenzier/assets/fqgan_triple_tiled_{tile_resize}"
            )
        else:
