...
class OpenMAGViT2(Tokenizer):
    ...
    def __init__(self, config_file: str, ckpt_path: str, name: str = "OpenMAGViT2_256", **kwargs):
    ...
    def encode(self, tensor: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, Any]]:
        ...
        with torch.no_grad():
            ...
            additional_info = {"quant": quant, "diff": diff, "original_shape": tensor.shape}
...
def reconstruction_tiled(tokenizer, processing_ratio, tile_resize, images, image_names):
    ...
    if processing_ratio == 1.0:
        ...
        RECONSTRUCTION_PATH = (
            f"/iopsstor/scratch/cscs/xyixuan/benchmark-image-tokenzier/assets/{tokenizer.name}_tiled_{tile_resize}"
        )
    else:
...
if __name__ == "__main__":
    ...
    for processing_ratio in processing_ratios:
        ...
        reconstruction_tiled(
            tokenizer_res_256, processing_ratio, tile_resize=256, images=images[1:], image_names=image_names[1:]
        )
