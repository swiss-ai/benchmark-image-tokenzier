...
class Tiler:
    ...
    def process(self, image: Union[torch.Tensor, Image.Image, np.ndarray]) -> Dict[str, Any]:
        ...
        downsampled = F.interpolate(
            image.unsqueeze(0), size=(scaled_H, scaled_W), mode="bicubic", align_corners=False, antialias=True
        ).squeeze(0)
        ...
        padded = F.pad(downsampled, (pad_left, pad_right, pad_top, pad_bottom), mode="constant", value=self.pad_value)
        ...
        print(
            f"Creating {num_tiles_H}×{num_tiles_W} = {num_tiles_H * num_tiles_W} tiles of {self.tile_resize}×{self.tile_resize}"
        )
    ...
    def reconstruct(self, tiles: torch.Tensor, metadata: Dict[str, Any]) -> torch.Tensor:
        ...
        # Step 1: Stitch tiles
        stitched = rearrange(tiles, "(nh nw) c th tw -> c (nh th) (nw tw)", nh=num_tiles_H, nw=num_tiles_W)
        ...
        reconstructed = F.interpolate(
            unpadded.unsqueeze(0), size=(orig_H, orig_W), mode="bicubic", align_corners=False, antialias=True
        ).squeeze(0)
    ...
    def show_tiles(self, tiles: torch.Tensor, metadata: Dict[str, Any], tight_layout: bool = True):
        ...
        ), f"Total tiles {total_tiles} doesn't match grid {num_H}×{num_W}={num_H*num_W}"
