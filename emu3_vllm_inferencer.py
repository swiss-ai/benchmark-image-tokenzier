...
class EMU3Inferencer:
    ...
    def predict_structure(self, height: int, width: int) -> Dict:
        ...
        # Generate
        result = self.generate(prompt_ids, max_tokens=8192, temperature=0.1)  # Let model decide when to stop
        ...
        analysis["structure_correct"] = (
            result["structure_tokens"].get("img_end_of_row", 0) == expected_eol
            and result["num_visual_tokens"] == height * width
        )
    ...
    def complete_image(self, height: int, width: int, given_rows: int = 1) -> Dict:
        ...
        # Add metadata tokens
        prompt_parts.extend(
            self.tokenizer.encode(f"<|img_start|>{height}*{width}<|img_token_start|>", add_special_tokens=False)
        )
        ...
        result = self.generate(
            prompt_parts, max_tokens=expected_remaining * 5, temperature=0.3  # Allow some extra for structure tokens
        )
    ...
    def visualize_tokens(self, visual_indices: List[int], height: int, width: int, title: str = "Visual Token Grid"):
        """
    ...
    def batch_generate(self, prompts: List[Union[str, List[int]]], **kwargs) -> List[Dict]:
        ...
        # Generate
        outputs = self.llm.generate(prompt_token_ids=prompt_ids_list, sampling_params=sampling_params)
