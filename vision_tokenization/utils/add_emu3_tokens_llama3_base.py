...
def add_emu3_special_tokens(
    model_path: str, output_path: Optional[str] = None, visual_vocab_size: int = 32768, num_reserved_tokens: int = 100
):
    """
    ...
    print(
        f"  <|RESERVED_{num_reserved_tokens-1:03d}|>: ID {tokenizer.convert_tokens_to_ids(f'<|RESERVED_{num_reserved_tokens-1:03d}|>')}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-path", type=str, required=True, help="Path to model/tokenizer directory")
    parser.add_argument(
        "--output-path", type=str, default=None, help="Path to save updated tokenizer (default: overwrite original)"
    )
    parser.add_argument(
        "--visual-vocab-size", type=int, default=32768, help="Number of visual tokens to add (default: 32768)"
    )
    parser.add_argument(
        "--num-reserved-tokens", type=int, default=100, help="Number of reserved tokens to add (default: 100)"
    )
    parser.add_argument("--example", action="store_true", help="Show example EMU3 sequence after updating")
