...
class TokenizationPlan:
    ...
    off_canonical_stats: Optional[dict] = None
...
def _plan_image_batches(
    ...
    *,
    ...
    exact_key_images = 0
    spillover_batches = 0
    off_aspect_pct: List[float] = []
    off_area_ratio: List[float] = []
    off_token_diff_pct: List[float] = []
    ...
    for wi in range(num_windows):
        ...
        if spillover_parts:
            ...
            batches, n_spill_batches, aspect_pct, area_ratio, token_diff_pct = _pack_spillover(
                spillover, comp_indices, final_h, final_w,
                batch_size=batch_size, max_batch_tokens=max_batch_tokens,
                spatial_factor=spatial_factor,
            )
            all_batches.extend(batches)
            spillover_batches += n_spill_batches
            off_aspect_pct.extend(aspect_pct)
            off_area_ratio.extend(area_ratio)
            off_token_diff_pct.extend(token_diff_pct)

    if stats_out is not None:
        off = np.asarray(off_aspect_pct, dtype=np.float64)
        area = np.asarray(off_area_ratio, dtype=np.float64)
        tok = np.asarray(off_token_diff_pct, dtype=np.float64)
        stats_out.update(
            total_images=int(N),
            exact_key_images=int(exact_key_images),
            off_canonical_images=int(off.size),
            off_canonical_pct=float(100.0 * off.size / N) if N else 0.0,
            spillover_batches=int(spillover_batches),
            exact_key_batches=int(len(all_batches) - spillover_batches),
            aspect_distortion_median_pct=float(np.median(off)) if off.size else 0.0,
            aspect_distortion_p99_pct=float(np.percentile(off, 99)) if off.size else 0.0,
            aspect_distortion_max_pct=float(off.max()) if off.size else 0.0,
            area_ratio_median=float(np.median(area)) if area.size else 1.0,
            token_count_changed_pct=float(100.0 * np.count_nonzero(tok) / tok.size) if tok.size else 0.0,
            token_count_diff_median_pct=float(np.median(tok)) if tok.size else 0.0,
            token_count_diff_p99_pct=float(np.percentile(tok, 99)) if tok.size else 0.0,
        )

    return all_batches, np.array(split_batch_offsets, dtype=np.int64)


def _off_canonical_records(
    members_arr: np.ndarray, sh: np.ndarray, sw: np.ndarray, rh: int, rw: int,
    *, spatial_factor: int,
) -> tuple:
    """Per-member aspect distortion (pct), area ratio, and vision-token-count
    diff (pct) for members whose cluster dims ``(rh, rw)`` differ from their own
    canonical dims. The token diff compares ``estimate_image_tokens`` at own dims
    vs cluster dims, measuring the actual sequence-length impact of the resize."""
    own_h = sh[members_arr].astype(np.float64)
    own_w = sw[members_arr].astype(np.float64)
    off = (own_h != rh) | (own_w != rw)
    if not off.any():
        return [], [], []
    oh, ow = own_h[off], own_w[off]
    aspect_pct = np.abs(np.log((rw / rh) / (ow / oh))) * 100.0
    area_ratio = (rh * rw) / (oh * ow)
    cluster_tok = estimate_image_tokens(rh, rw, spatial_factor=spatial_factor)
    own_tok = np.array([
        estimate_image_tokens(int(h), int(w), spatial_factor=spatial_factor)
        for h, w in zip(oh, ow)
    ], dtype=np.float64)
    token_diff_pct = np.abs(cluster_tok - own_tok) / own_tok * 100.0
    return aspect_pct.tolist(), area_ratio.tolist(), token_diff_pct.tolist()
...
def _pack_spillover(
    ...
    *,
    ...
    Returns ``(batches, n_batches, off_aspect_pct, off_area_ratio, off_token_diff_pct)``
    where the three off-canonical lists are measurement-only and do not affect batching.
    """
    import faiss
    ...
    if k <= 1:
        ...
        members = np.arange(N)
        batches = _make_batches(members, avg_h, avg_w)
        off_aspect, off_area, off_token = _off_canonical_records(
            members, sh, sw, avg_h, avg_w, spatial_factor=spatial_factor,
        )
        return batches, len(batches), off_aspect, off_area, off_token
    ...
    off_aspect: List[float] = []
    off_area: List[float] = []
    off_token: List[float] = []
    for cid in range(k):
        ...
        cluster_aspect, cluster_area, cluster_token = _off_canonical_records(
            members, sh, sw, avg_h, avg_w, spatial_factor=spatial_factor,
        )
        off_aspect.extend(cluster_aspect)
        off_area.extend(cluster_area)
        off_token.extend(cluster_token)
...
def build_plan_alignment(
    ...
    *,
    ...
    off_canonical_stats: dict = {}
    image_batches, split_batch_offsets = _plan_image_batches(
        doc_ids, doc_ids, widths, heights, doc_ids,
        batch_size=batch_size, max_batch_tokens=max_batch_tokens,
        spatial_factor=spatial_factor, resize_min_pixels=resize_min_pixels,
        resize_max_pixels=resize_max_pixels, window_size=window_size,
        stats_out=off_canonical_stats,
    )
    plan.execution.image_batches = ImageBatchTable.from_batches(image_batches)
    plan.execution.split_batch_offsets = split_batch_offsets
    plan.off_canonical_stats = off_canonical_stats
