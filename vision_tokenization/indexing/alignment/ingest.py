...
def _task_payload_columns(task: str, schema: pa.Schema) -> list[str]:
    if task == "preference":
        ...
        optional = ["answer_variants", "source-id"]
    else:
        required = []
        optional = ["source-id"]
...
def _normalize(messages, *, allow_system: bool = False):
    out = []
    for m in messages:
        if m["role"] == "system" and not allow_system:
...
def _parse_rl_prompt_row_with_refs(row: dict, refs: list[str]) -> dict:
    """Parse one rl_prompt-shaped row using already-computed media refs.

    Mirrors the preference parser collapsed to prompt-only; allows a system
    turn (real RL data keeps a system instruction the chat template renders).
    """
    prompt = _normalize(row["prompt"], allow_system=True)
    ...
    if n_markers != len(refs):
        raise MarkerMismatch(
            f"{row.get('source-id')}: {n_markers} markers vs {len(refs)} images")
    if MARKER in answer:
        raise MarkerMismatch("accidental marker in answer")
    return {
        "prompt": prompt,
        "answer": answer,
        "answer_variants": list(row.get("answer_variants") or []),
        "prompt_media_refs": refs,
        "chosen_media_refs": [], "rejected_media_refs": [],
        "prompt_id": str(row.get("source-id", "")),
    }
...
# A third task should fold {required_cols, parser} into one per-task descriptor.
_PARSERS = {
    "preference": _parse_preference_row_with_refs,
    "rl_prompt": _parse_rl_prompt_row_with_refs,
}
