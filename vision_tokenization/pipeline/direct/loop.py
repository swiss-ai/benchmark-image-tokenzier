...
from vision_tokenization.utils.json import json_dumps
import logging
...
def _load_or_compute_batch_plan(cfg: Dict[str, Any], is_multi_image: bool) -> BatchPlan:
    ...
    batch_plan = cfg.get("batch_plan")
    if batch_plan and Path(batch_plan).exists():
        logger.info(f"Loading pre-computed batch plan from {batch_plan}")
        ...
        plan = torch.load(batch_plan, map_location="cpu", weights_only=False)
    ...
    locality_cfg = cfg.get("locality") or {}
    use_locality = locality_cfg.get("enabled", False)

    if use_locality:
        logger.info("Using locality-aware batch planner")
        plan = plan_locality_batches(
            ...
            window_size=locality_cfg.get("window_size", 5000),
        )
    else:
    ...
    if batch_plan:
        Path(batch_plan).parent.mkdir(parents=True, exist_ok=True)
        torch.save(plan, batch_plan)
        logger.info(f"Saved batch plan to {batch_plan}")
...
def _append_rank_stats(stats_path: Path, result: Dict[str, Any]) -> None:
    ...
    line = json_dumps(result, default=str) + "\n"
