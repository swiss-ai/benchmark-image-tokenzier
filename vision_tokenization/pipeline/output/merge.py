...
def _strip_thinking_inner(tokens, think_id, end_think_id, out):
    """Numba-compiled one-pass state machine. Returns (write count, ended inside)."""
    w = 0
    inside = False
...
def strip_thinking_tokens(
    ...
    ``think_id``/``end_think_id`` must come from the tokenizer that produced
    *tokens* (see :func:`resolve_thinking_token_ids`); delimiter IDs drift
    across vocab generations — under the v2 unified vocab the former defaults
    32/33 are ``<|img_generation_start|>``/``<|audio_start|>``, which turned
    every image-generation span into an unterminated "thinking" span and
    silently deleted the rest of the sequence.
    ...
    w, unterminated = _strip_thinking_inner(tokens, think_id, end_think_id, out)
    if unterminated and not allow_unterminated:
        raise ValueError(
            f"unterminated thinking span (opened by id {think_id}, never closed by "
            f"id {end_think_id}): corrupt sequence or wrong delimiter IDs for this "
            "tokenizer; pass allow_unterminated=True only if truncated generations "
            "are expected"
        )
