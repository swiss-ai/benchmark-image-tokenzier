...
def _tokenized_maps(table: pa.Table) -> tuple[dict, dict]:
    ...
    The rl_prompt spill has no response columns; default chosen/rejected to 0 so the
    3-tuple shape (and the merge's seq arithmetic) stays uniform across tasks."""
    pids = table.column("prompt_id").to_pylist()
    ptl = pc.list_value_length(table.column("prompt_text_ids")).to_pylist()
    if "chosen_ids" in table.column_names:
...
class AlignmentPayloadBackend:
    ...
    def _build_view_rows(self, split: str) -> tuple[list[dict], int]:
        ...
        build_row = self._preference_row if self._task == "preference" else self._rl_prompt_row
        view_rows = []
        dropped = 0
        for row in self._rows_by_split[split]:
            ...
            view_rows.append(build_row(row, images))
    ...
    def _rl_prompt_row(self, row: dict, images: list[dict]) -> dict:
        prompt_id = str(row.get("prompt_id", ""))
        media_tokens_total = sum(int(img["token_length"]) for img in images)
        si = self._seq_input(prompt_id)
        prompt_len = seq_lengths(si[0], media_tokens_total, 0, 0)[0]

        prompt = row.get("prompt") or []
        answer = str(row.get("answer", ""))
        answer_variants = [str(v) for v in (row.get("answer_variants") or [])]
        text_chars = row.get("text_chars")
        if text_chars is None:
            text_chars = (
                sum(len(str(m.get("content", ""))) for m in prompt)
    ...
    def finalize(self) -> None:
        view_schema = VIEW_SCHEMA if self._task == "preference" else RL_PROMPT_VIEW_SCHEMA
        files: dict[str, int] = {}
