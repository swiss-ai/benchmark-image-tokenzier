...
def tokenize_pair_views(views_path, spill_dir, rank: int, world_size: int,
                        *, tokenizer_path, system: str = "empty", task: str = "preference") -> int:
    """Tokenize this rank's disjoint slice of alignment rows from ``views_path`` and
    spill to ``<spill_dir>/views_tokenized/rank_NNNN.parquet``. CPU-only and vision-free;
    safe to run concurrently with the rank's GPU vision encode. The per-row body and the
    spill schema fork on ``task`` (``preference`` pairs vs ``rl_prompt`` prompt-only);
    the slot-count check is shared. Returns the row count."""
    ...
    if task == "preference":
        ...
        schema = TOKENIZED_RL_PROMPT_SCHEMA
    else:
    ...
    for row in mine.to_pylist():
        if task == "preference":
            ...
            spill = tokenized_to_row(tp, row["prompt_id"])
        else:
            tp = tokenize_rl_prompt_text(row, tok, policy, markers)
            spill = prompt_to_row(tp, row["prompt_id"])
    ...
    _write_parquet_atomic(pa.Table.from_pylist(rows, schema=schema), out_dir / f"rank_{rank:04d}.parquet")
    return len(rows)
...
def main(argv=None) -> int:
    ...
    p.add_argument("--task", default="preference")
    a = p.parse_args(argv)
    n = tokenize_pair_views(a.views_path, a.spill_dir, a.rank, a.world_size,
                            tokenizer_path=a.tokenizer_path, system=a.system, task=a.task)
    print(f"alignment text pass: rank {a.rank} tokenized {n} rows")
    return 0
