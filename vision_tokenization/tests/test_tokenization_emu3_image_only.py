...
def image_tokenizer(temp_tokenizer):
    ...
    return EMU3ImageOnlyTokenizer(text_tokenizer_path=temp_tokenizer, min_pixels=384 * 384, max_pixels=1024 * 1024)
...
class TestEMU3ImageOnlyTokenizer:
    ...
    def test_tokenize_2x2_image(self, image_tokenizer):
        ...
        tokens = image_tokenizer.encapsulate_image(image_indices, height, width)
        ...
        assert (
            vision_token_ids == expected_vision_ids
    ...
    def test_vision_token_offset(self, image_tokenizer):
        ...
        tokens = image_tokenizer.encapsulate_image(image_indices, 1, 1)
    ...
    def test_dimension_encoding(self, image_tokenizer):
        ...
        for height, width in test_dims:
            ...
            dim_tokens = image_tokenizer.text_tokenizer.encode(dim_text, add_special_tokens=False)
    ...
    def test_no_generation_token_for_image_only(self, image_tokenizer):
        ...
        tokens = image_tokenizer.encapsulate_image(image_indices, 2, 2)
    ...
    def test_compare_with_original(self, image_tokenizer):
        ...
        for height, width, image_indices in test_cases:
            comparison = compare_with_original(image_tokenizer, image_indices, height, width)
            ...
            # Results must match
            assert comparison["tokens_match"], f"Token mismatch for {height}x{width}! Direct != Text-based"
            ...
            assert torch.equal(direct_tokens, text_based_tokens), f"Tokens don't match for {height}x{width}"
...
class TestIntegration:
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                image_tokenizer = EMU3ImageOnlyTokenizer(
                    text_tokenizer_path=temp_dir, min_pixels=384 * 384, max_pixels=1024 * 1024
                )

                # Test basic tokenization
                indices = torch.tensor([0, 1, 2, 3])
                tokens = image_tokenizer.encapsulate_image(indices, 2, 2)
    ...
    def test_with_qwen_tokenizer(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                add_emu3_special_tokens(
                    model_path="Qwen/Qwen2-7B", output_path=temp_dir, visual_vocab_size=100, num_reserved_tokens=10
                )
                ...
                result = image_tokenizer.tokenize_batch(batch, dims, padding=True)
