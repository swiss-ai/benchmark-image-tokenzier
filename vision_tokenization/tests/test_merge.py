...
class TestStripThinkingTokens:
    def test_basic(self):
        ...
        result = strip_thinking_tokens(tokens, THINK, END_THINK)
    ...
    def test_unmatched_open_raises(self):
        ...
        with pytest.raises(ValueError, match="unterminated thinking span"):
            strip_thinking_tokens(tokens, THINK, END_THINK)

    def test_unmatched_open_allowed(self):
        tokens = np.array([1, THINK, 99, 100], dtype=np.int32)
        result = strip_thinking_tokens(tokens, THINK, END_THINK, allow_unterminated=True)
