...
from add_emu3_tokens_llama3_base import add_emu3_special_tokens, get_vision_token_id, load_vision_token_mapping
...
@pytest.fixture(params=["meta-llama/Meta-Llama-3-8B", "Qwen/Qwen2-7B", "alehc/swissai-tokenizer", "gpt2"])
...
def small_tokenizer(temp_tokenizer_path, base_model):
    ...
    tokenizer, stats = add_emu3_special_tokens(
        model_path=base_model, output_path=output_path, visual_vocab_size=100, num_reserved_tokens=20
    )
...
class TestTokenizerUpdate:
    ...
    def test_add_reserved_tokens(self, temp_tokenizer_path, base_model):
        ...
        tokenizer, stats = add_emu3_special_tokens(
            model_path=base_model, output_path=output_path, visual_vocab_size=50, num_reserved_tokens=num_reserved
        )
    ...
    def test_add_visual_tokens(self, temp_tokenizer_path, base_model):
        ...
        tokenizer, stats = add_emu3_special_tokens(
            model_path=base_model, output_path=output_path, visual_vocab_size=visual_vocab_size, num_reserved_tokens=10
        )
    ...
    def test_vocab_size_increase(self, temp_tokenizer_path, base_model):
        ...
        expected_increase = (
            stats["structure_tokens_added"] + stats["reserved_tokens_added"] + stats["visual_tokens_added"]
        )
...
class TestFullScale:
    ...
    def test_full_emu3_tokenizer(self, temp_tokenizer_path, base_model):
        ...
        tokenizer, stats = add_emu3_special_tokens(
            model_path=base_model, output_path=output_path, visual_vocab_size=32768, num_reserved_tokens=100
        )
...
def test_specific_models(model_name, temp_tokenizer_path):
    ...
    tokenizer, stats = add_emu3_special_tokens(
        model_path=model_name, output_path=output_path, visual_vocab_size=100, num_reserved_tokens=20
    )
