...
Both writers allocate from this, the planner predicts from it,
and the numba estimator freezes its constants at compile time.
Kept free of torch and numba so the planner, which is offline, can import it.
...
IMAGE_BLOCK_FIXED_TOKENS = 4
SEQUENCE_WRAPPER_TOKENS = 2
...
def dim_tokens_upper_bound(height: int, width: int) -> int:
    ...
    so the character count bounds the token count from above for any tokenizer
    that does not split a character — byte-level BPE only ever merges.
    It happens to be exact for the Apertus tokenizers, which keep digits separate.

    The planner has no tokenizer and must bound rather than measure.
    Over-estimating only makes batches slightly smaller than they could be;
    under-estimating makes them overrun max_batch_tokens.
    """
