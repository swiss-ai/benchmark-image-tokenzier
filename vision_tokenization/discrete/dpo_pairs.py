...
def render_prompt_prefix(prompt, tok, policy, markers, *, enable_thinking):
    """Render the prompt-prefix (chat template up to the assistant generation point),
    split its image slots out, and tokenize the text spans. Returns
    ``(norm_prompt, rendered, prompt_text_ids, image_insert_positions)``. ``rendered``
    is returned because the caller's prompt-prefix stability check needs it.
    Vision-free: image markers become positional slots into ``prompt_text_ids``."""
    norm_prompt = apply_conversation_policy([dict(m) for m in prompt], policy)
    ...
    return norm_prompt, rendered, prompt_text_ids, image_insert_positions
...
def tokenize_pair_text(row, tok, policy, markers) -> TokenizedPair:
    ...
    norm_prompt, rendered, prompt_text_ids, image_insert_positions = render_prompt_prefix(
        row["prompt"], tok, policy, markers, enable_thinking=enable_thinking)
...
def pair_lengths(tp: TokenizedPair, image_token_lengths, *, prompt_id: str = "") -> dict:
    ...
    for pos, ln in zip(tp.image_insert_positions, image_token_lengths):
        ...
        cum_vision += ln
    chosen_len = int(len(getattr(tp, "chosen_ids", ())))
    rejected_len = int(len(getattr(tp, "rejected_ids", ())))
...
class TokenizedPrompt:
    """One rl_prompt sample tokenized at the text level — the [prompt] doc, no
    response. Shares ``prompt_text_ids``/``image_insert_positions`` attribute names
    with ``TokenizedPair`` so the slot-count check and splice are the same code."""

    prompt_text_ids: np.ndarray
    image_insert_positions: list[int]


def tokenize_rl_prompt_text(row, tok, policy, markers) -> TokenizedPrompt:
    """Render the rl prompt-prefix with ``enable_thinking`` pinned True (a task
    constant, never read from the row) and tokenize its text spans. Vision-free:
    image markers become positional slots."""
    norm_prompt, rendered, prompt_text_ids, image_insert_positions = render_prompt_prefix(
        row["prompt"], tok, policy, markers, enable_thinking=True)

    # The only guard the stored [prompt] tail matches the tokenizer's own rendering:
    # segment-wise tokenization (per text span) can drift from whole-string tokenization
    # at a BPE boundary across the image slots. DPO catches this via the appended
    # response (full[:p] == prompt_token_ids); rl has no response, so check directly.
    # <|image|> is one token / hard boundary in the whole render, so the correct
    # invariant is segment-wise == whole-with-markers minus the image token at each slot;
    # joining the text spans (dropping the boundary) would let BPE merge across the slot.
    image_id = tok.convert_tokens_to_ids("<|image|>")
    whole = [t for t in tok(rendered, add_special_tokens=False)["input_ids"] if t != image_id]
    if whole != prompt_text_ids.tolist():
        raise ValueError(f"{row.get('prompt_id', '')}: prompt prefix not stable under apply_chat_template")
...
def assemble_prompt_only(tp: TokenizedPrompt, images, tokens, *, prompt_id: str = ""):
    """Return (prompt_ids:int32 np.ndarray, index_row:dict): splice each media's
    deduped vision block into the prompt slots; the whole doc IS the prompt. Reuses
    ``pair_lengths`` (chosen_len=rejected_len=0) so offsets share one arithmetic.
    ``answer``/``enable_thinking`` are added at binidx, not here."""
    idx = pair_lengths(tp, [int(im["token_length"]) for im in images], prompt_id=prompt_id)
    parts, prev = [], 0
    for pos, im in zip(tp.image_insert_positions, images):
        parts.append(tp.prompt_text_ids[prev:pos])
        off, ln = int(im["token_offset"]), int(im["token_length"])
        ...
        prev = pos
    parts.append(tp.prompt_text_ids[prev:])
    prompt_ids = np.concatenate(parts) if parts else np.empty(0, dtype=np.int32)
    if len(prompt_ids) != idx["prompt_len"]:
        raise RuntimeError(
            f"{prompt_id}: spliced prompt {len(prompt_ids)} != layout {idx['prompt_len']}")
    index_row = {
        "prompt_id": prompt_id,
        "prompt_len": idx["prompt_len"],
        "image_offsets": idx["image_offsets"],
        "image_lengths": idx["image_lengths"],
        "image_tok": idx["image_tok"],
    }
    return prompt_ids, index_row


def prompt_to_row(tp: TokenizedPrompt, prompt_id: str) -> dict:
    """Flatten a ``TokenizedPrompt`` for the engine spill (``TOKENIZED_RL_PROMPT_SCHEMA``).
    Pairs with ``prompt_from_row``."""
    return {
        "prompt_id": prompt_id,
        "prompt_text_ids": tp.prompt_text_ids.tolist(),
        "image_insert_positions": [int(x) for x in tp.image_insert_positions],
...
def prompt_from_row(row: dict) -> TokenizedPrompt:
    """Rebuild a ``TokenizedPrompt`` from a spilled row (the merge/binidx side)."""
    return TokenizedPrompt(
        prompt_text_ids=np.asarray(row["prompt_text_ids"], dtype=np.int32),
        image_insert_positions=[int(x) for x in row["image_insert_positions"]],
    )
