...
def create_webdataset_shard(
    images: List[Tuple[str, Image.Image]], output_path: str, vision_tokenizer, text_tokenizer, shard_id: int = 0
):
        ...
        for idx, (name, image) in enumerate(images):
            ...
            sample = {
                ...
                "indices.pyd": pickle.dumps(
                    {"indices": sample_data["indices"], "height": sample_data["height"], "width": sample_data["width"]}
                ),
                "metadata.json": {
                    "original_name": name,
...
def process_dataset_distributed(
    image_dir: str, output_dir: str, tokenizer_path: str, images_per_shard: int = 1000, num_shards: int = None
):
    ...
    for img_path in image_files:
        try:
            ...
            if len(current_batch) >= images_per_shard:
                create_webdataset_shard(current_batch, output_dir, vision_tokenizer, text_tokenizer, shard_id)
                shard_id += 1
                current_batch = []
...
def verify_webdataset_shard(shard_path: str, tokenizer_path: str):
    ...
    for sample in dataset:
        ...
        recreated = text_tokenizer.tokenize_image_only(indices, indices_data["height"], indices_data["width"])
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if args.mode == "prepare":
                ...
                process_dataset_distributed(
                    args.image_dir, args.output_dir, args.tokenizer_path, args.images_per_shard, args.num_shards
                )
