#!/usr/bin/env python3
"""
Ultra-fast parallel checksum verification for HuggingFace Hub cache files.
...
def find_cache_blobs(cache_dir: Path, dataset_name: str = None) -> List[Path]:
    """Find all blob files in the HuggingFace cache."""
    blobs = []
...
def verify_cache_optimized(cache_dir: Path, dataset_name: str = None, 
    ...
    """
    Verify all blobs using optimized parallel processing with batching.
...
def list_datasets(cache_dir: Path):
    ...
    for dataset_dir in sorted(dataset_dirs):
        ...
        if blob_dir.exists():
            blob_count = len(list(blob_dir.iterdir()))
            print(f"  {dataset_name}: {blob_count} files")
        else:
            print(f"  {dataset_name}: 0 files")
